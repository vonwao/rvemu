...
class rvemu(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","rvemu")
        # Bare-metal toolchain prefix (xPack riscv-none-elf, pinned in targets/PINS.md)
        self.cc_prefix = config['ccprefix'] if 'ccprefix' in config else 'riscv-none-elf-'
    ...
    def initialise(self, suite, work_dir, archtest_env):
       ...
       # runTests function
       self.compile_cmd = self.cc_prefix + 'gcc -march={0} \
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for rvemu start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
    ...
    def runTests(self, testList):
	  ...
	  # echo statement.
          if self.target_run:
            # rvemu CLI contract per harness/README.md
            simcmd = self.dut_exe + ' --max-insns 30000000 --signature {0} --signature-granularity 4 {1}'.format(sig_file, elf)
          else:
            simcmd = 'echo "NO RUN"'
